...
const renderRegistro = () => renderWithProviders(<Registro />);
...
describe("Registro — renderizado", () => {
  ...
  it('muestra el campo de monto con placeholder "0"', () => {
    renderRegistro();
...
describe("Registro — validación del monto", () => {
  ...
  it("muestra error si el monto está vacío", () => {
    renderRegistro();
    fireEvent.change(screen.getByPlaceholderText("0"), {
      target: { value: "" },
    });
  ...
  it("acepta un monto entero válido y lo formatea con puntos de miles (COP)", () => {
    renderRegistro();
    const input = screen.getByPlaceholderText("0");
  ...
  it("formatea correctamente un monto grande con puntos de miles", () => {
    renderRegistro();
    const input = screen.getByPlaceholderText("0");
...
describe("Registro — flujo feliz", () => {
  ...
  it("el error desaparece al corregir el monto", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(/ingresa un monto válido/i),
...
describe("Registro — selección de categoría", () => {
  it('la categoría "Comida" está activa por defecto', () => {
    renderRegistro();
    expect(screen.getByRole("button", { name: /comida/i })).toHaveClass(
      "rg-cat--active",
    );
  });
  ...
  it("cambia la categoría activa al hacer clic en otra", () => {
    ...
    expect(screen.getByRole("button", { name: /transporte/i })).toHaveClass(
      "rg-cat--active",
    );
  });
  ...
  it("desactiva la categoría anterior al seleccionar una nueva", () => {
    ...
    expect(screen.getByRole("button", { name: /diversión/i })).toHaveClass(
      "rg-cat--active",
    );
    expect(screen.getByRole("button", { name: /comida/i })).not.toHaveClass(
      "rg-cat--active",
    );
  });
});
...
describe("Registro — selector de tipo de movimiento", () => {
  ...
  it('cambia a ingreso al hacer clic en el botón "Ingreso"', async () => {
    ...
    const toggleIngreso = botonesIngreso.find((b) =>
      b.className.includes("rg-type-btn"),
    );
